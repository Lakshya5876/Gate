...
The first job step checks that `.githooks/verify_governance_integrity.sh` is present, then delegates to it. That script (`templates/verify_governance_integrity.sh`, deployed by `install.sh` alongside `gate.sh`) checks that `.claude/gate_state.json` and `.claude/gate_integrity.sha256` are present, then runs `sha256sum -c` (or `shasum -a 256 -c` on macOS) against the manifest in `.claude/gate_integrity.sha256` — a content check, not just a presence check, since presence alone lets a PR weaken a governance script and still pass CI (Module A6/A7). As of Module A10, the manifest is multi-file, not single-file: it covers `.githooks/gate.sh`, `.githooks/verify_governance_integrity.sh`, `.githooks/pre-commit`, `.githooks/pre-push`, and `.claude/hooks/pre_bash_trust_root_guard.sh` in one `sha256sum -c` call, so weakening *any* script in the enforcement chain — not just `gate.sh` — is caught the same way. This prevents a PR that deletes, renames, or silently weakens any of these files from disabling CI governance. If a PR removes or tampers with any of them, CI fails loudly before the gate even runs.
...
## A10. Exhaustive Ownership-Test Pass — Closing Every Remaining Gap, Collapsing the Last Duplication (2026-07-02)

*Changes made in the session after A9 was committed, in response to a sixth-pass audit that applied A9's own justification test to every other install.sh-generated artifact and found it hadn't been applied exhaustively.*

### The Findings

A9 protected `.github/workflows/gate.yml` using a specific, reusable test: *"is this file install.sh-owned, and does the agent ever need to create it through Claude Code's own Write/Edit tools?"* If no, it's safe to lock from install time. A sixth-pass audit applied that exact test to every other artifact `install.sh` generates and found it hadn't been applied everywhere:

1. **`.claude/hooks/pre_bash_trust_root_guard.sh` — the Bash-guard hook's own script — was undefended.** Disclosed by the audit as the sharpest finding: an agent could overwrite the guard with a one-line no-op while its `hooks.PreToolUse` registration in `settings.json` stayed intact, making the control look active while doing nothing. Stealthier than any bypass the guard itself was built to close, since a reviewer checking `settings.json` would see no evidence anything was wrong.
2. **The dual JSON-literal / Python-literal deny-list was flagged as a live recurrence of the exact "two hand-synced copies" shape that caused the CI/test drift fixed in Module A8** — currently identical, but nothing mechanically prevents the next entry from being added to only one.

Independently, while applying the same ownership test exhaustively rather than stopping at the disclosed finding, two more gaps were found that the audit hadn't surfaced:

3. **`.claude/gate_state.json` — the gate's own ledger — was unprotected.** Receipts, token spend, the audit log, and the `claude_md_hash` drift marker all live here. An agent that can `Write`/`Edit` it directly can fabricate a passing receipt or reset its own token budget, which would defeat every other control in this entire chain — arguably more severe than the guard-script gap, since it doesn't require understanding the enforcement mechanism at all, just editing the ledger to say whatever is convenient.
4. **`.mcp.json` — the MCP server configuration — was unprotected.** Controls which MCP servers Claude Code connects to; install.sh-owned and static after install, never legitimately agent-edited.

### The Fixes

**Trust-root protection extended** to all four: `.claude/hooks/**` (directory-level glob, matching the existing `.githooks/**` pattern — self-protecting, and future-proofs against any hook script added to that directory later), `.claude/gate_state.json`, `.mcp.json`. Added to both the native `permissions.deny` array and the Bash-guard hook's own `PROTECTED_PATHS`. `.claude/gate_state.json`'s denial has zero effect on `gate.sh`'s own legitimate mutations — those run via a git-hook subprocess calling `python3` directly, never through Claude Code's tool-mediated Write/Edit, so only the *agent's own* tool calls are affected.

**The dual deny-list collapsed to one canonical Python list.** `_write_trust_root_settings()` now contains exactly one `REQUIRED_DENY` list, read by a single `python3` heredoc that branches on `os.path.exists('.claude/settings.json')` — fresh-write and idempotent-merge are now two code paths through one script, not two hand-authored copies of the same data. There is nothing left to keep in sync by hand.

**The CI integrity manifest extended from one file to five.** `.claude/gate_integrity.sha256` is now a multi-file manifest in `sha256sum`'s native check format (`<hash>  <path>` per line — verified cross-compatible between `sha256sum -c` and `shasum -a 256 -c` before committing to the design), covering `.githooks/gate.sh`, `.githooks/verify_governance_integrity.sh`, `.githooks/pre-commit`, `.githooks/pre-push`, and `.claude/hooks/pre_bash_trust_root_guard.sh`. `verify_governance_integrity.sh` was rewritten to check all five with a single `sha256sum -c` / `shasum -a 256 -c` call instead of a hand-rolled single-file comparison — this is what actually closes the guard-script gap at the CI layer, not just the agent-tool layer, since a human contributor's direct commit (outside Claude Code's permission system entirely) would bypass the deny-list but not a CI-side hash mismatch.

**`pre-commit` and `pre-push` extracted to `templates/`.** They were fully static heredocs inside `install.sh` (no per-repo interpolation — confirmed by inspecting the quoted heredoc delimiters before extracting) with no reason to stay inlined. This was done specifically so the bats test fixture (`setup_gate_repo`) could deploy the exact real files instead of a hand-copy, achieving full fidelity for the manifest tests without any risky restructuring of `install.sh`'s own execution model — sourcing `install.sh` directly for testing was considered and rejected, since its top-level code (interactive prompts, preflight checks) isn't guarded behind a sourcing check, and adding that guard risked introducing a new bug into the installer to fix a test-fixture problem.

### A Real Bug Caught by Actually Running the Tests, Not by Review

While rewriting `ci_integrity_check.bats` for the new manifest, a debugging session was required: four new/updated tests failed with `[[ "$output" == *"does not match the pinned integrity manifest"* ]]` even though the captured output visibly contained that exact phrase. Manual reproduction outside bats worked correctly; the same assertion inside bats failed. Root cause, found by printing `$output` verbatim via `>&3` inside the failing test rather than by re-reading the code: the actual script text is *"governance files **do** not match"* (grammatically correct plural agreement with "files"), while the test assertion checked for *"**does** not match"* (singular) — a wording mismatch between the script and the test that would have shipped as a silently-mis-scoped test (one that happens to pass when it shouldn't, or fails when the real bug isn't present) had the suite not actually been run to completion. Fixed by aligning the assertions to the script's real text. This is exactly the class of mistake "review looks right" cannot catch and "run it" always does.

### Verification

Every fix in this addendum was verified by direct execution, not by re-reading the diff:
- `_write_trust_root_settings()` re-extracted into an isolated scratch directory and run for both the fresh-write and idempotent-merge branches; confirmed all 8 new deny entries present, `defaultMode` intact, no duplicate `PreToolUse` hook entries across repeated runs, and a hand-added allow-list/self-lock survives a second run untouched.
- The Bash guard tested against four realistic commands: overwriting itself with a no-op (the exact disclosed stealth-neuter bypass) — blocked; direct `gate_state.json` tampering — blocked; `.mcp.json` rewrite — blocked; a legitimate `cat .claude/settings.json` (needed by the init prompt) — still allowed. A deliberately obfuscated `chr()`-constructed path bypassed the guard, as expected and already documented as a known, disclosed limitation of lexical command-text scanning — not a new gap.
- `sha256sum -c` / `shasum -a 256 -c` cross-compatibility verified directly (generate with one tool, verify with the other, confirm tamper detection) before adopting the manifest format.
- Full bats suite: 26/26 passing, including three new tests exercising the exact bypass this addendum closes (`CI integrity check blocks when the Bash-guard hook is tampered with (stealth-neuter case)`, plus coverage for `pre-commit`/`pre-push` tampering).

### Two Adjacent Gaps Fixed While Present, Not Folded Into Scope Creep

While updating the STEP 4 ACTIVATION commit checklist in the brownfield implementation package to include `.claude/hooks/`, `.mcp.json` and `.github/workflows/gate.yml` were found to have never been listed in that checklist at all — meaning a human following it exactly could complete setup without ever committing two install.sh-generated governance files. Fixed in the same pass since it was directly adjacent to the file already being edited for this addendum's own purpose, not an unrelated expansion.

### What Six Rounds of This Cycle Demonstrate

Audit → verify independently → fix → re-audit → find the next-layer gap via the *same reasoning the previous fix used* → verify by execution, not review → document. Six consecutive rounds on this framework, each closing a real gap without introducing a regression that survived to the next round (the two regressions that did occur — the four bats tests broken by Fix #2's earlier ordering change, and the grammar-mismatch test bug in this round — were both caught by actually running the suite before considering the work done, not by re-reading the diff). The trust-root chain, as of this addendum, protects every install.sh-owned governance artifact this framework generates: the git hooks, the CI workflow, the integrity manifest, the Bash-guard hook that enforces all of it, the ledger the whole system depends on for its own audit trail, and the MCP configuration — using exactly one canonical list for the native deny-list and exactly one script for the CI/test integrity check, with nothing left duplicated by hand.

---

## A11. Two Independent Re-Audits, Two Concrete Findings, Both Fixed (2026-07-02)

*Changes made in the session after A10 was committed. Two independent audit passes of commit `13d6bd5` were run and compared side by side rather than trusting either one alone — they scored the commit differently (8.1 vs 8.3) but converged on largely the same substance, and each surfaced one concrete, independently-verifiable finding the other didn't.*

### Finding 1 (from the more detail-oriented pass): The Bash Guard's Own Logic Had Zero Behavioral Test

`tests/gate/ci_integrity_check.bats`'s fixture generated `.claude/hooks/pre_bash_trust_root_guard.sh` as a two-line stub (`#!/usr/bin/env bash` + `exit 0`) purely so the integrity-manifest mechanism had a file to hash. Verified directly: grepped every `.bats` file for the guard's own filename and confirmed no test ever fed it a real command and checked whether it actually blocked or allowed anything. The most novel, most security-critical piece of logic added across A7–A10 — the one thing standing between "an agent tries a redirection trick" and "the trick works" — had only ever been manually verified in scratch directories during development, never locked into the committed suite.

**The fix required a structural change, not just a new test file.** The guard's content lived as an inline `install.sh` heredoc with two bash-interpolated variables (`${dev_guide_dst}`, `${init_pkg_dst}`) — testing the *real* logic meant either hand-copying that logic into a test fixture (reintroducing the exact duplication class Module A8 eliminated) or extracting it to a genuine template. Chose the latter: `templates/pre_bash_trust_root_guard.sh` now exists with `__DEV_GUIDE_DST__`/`__INIT_PKG_DST__` placeholder tokens, substituted by `install.sh` via `sed -i.bak` (verified cross-platform-safe — `-i.bak` as a single token works identically on BSD sed and GNU sed, unlike the classic `-i ''` trap; confirmed by direct test before committing to the design) after `_fetch`. `tests/gate/test_helper.bash` gained `deploy_bash_guard()` (deploys the real template with test substitutions) and `run_bash_guard()` (feeds it a synthetic PreToolUse JSON payload exactly as Claude Code would). `tests/gate/bash_guard_logic.bats` (12 tests) then exercises the actual matching logic: six bypass attempts across every protected path (including the exact stealth-neuter case, a `tee`-based write to prove it's not redirection-specific, and gutting the CI workflow via `sed`), and five legitimate-access checks (reading/programmatically-editing `settings.json`, writing `CLAUDE.md`, an unrelated benign command, and — inverted — confirming the reference-doc filenames *are* correctly blocked, asserting documented behavior rather than assuming it).

All 12 passed on the first real run against the actual template — meaning the logic was correct all along; what was missing was proof of that fact that survives the next edit.

### Finding 2 (from the other pass): "Exhaustive" Coverage Was a Manual Audit Exercise, Not a Mechanically-Enforced Invariant

Both A9 and A10 were triggered by a human (an audit pass) noticing a file install.sh generates that wasn't in the deny-list. Nothing in the codebase would catch a *fifth* such omission automatically — the next one would need a seventh audit round to find by inspection, the same way the first four were found.

**Built the missing mechanism.** `tests/gate/deny_list_coverage.bats` (4 tests) runs the *actual* `_write_hooks()` and `_write_trust_root_settings()` functions — extracted from the real `install.sh` via a robust helper, `extract_install_functions()` in `test_helper.bash`. That extraction is anchored on function-**start** markers (`grep -n '^_write_hooks() {'` through the line before `^_upgrade() {`), deliberately not on a closing-brace search: the settings.json heredoc these functions write contains a `}` at column 0 (the JSON object's own closing brace), which silently truncates any extraction searching for `^}$` as an end marker — confirmed this failure mode directly with an early attempt before settling on start-anchored extraction. After running the real functions in a scratch directory, the four tests walk the actual files written to `.githooks/` and `.claude/hooks/` and assert every one is covered by the generated deny-list; cross-check that the integrity manifest covers the same set in the other direction; and assert the two top-level-written files (`.github/workflows/gate.yml`, `.mcp.json`) that aren't isolable through these two functions are covered via a named list. A file added to either directory without matching deny-list coverage now fails a test immediately on the next `pytest`-equivalent run, not on the next audit pass.

### A Real, Reproducible Debugging Detour: Manual `source` in the Tool Shell Doesn't Populate `BASH_SOURCE[0]`

While building `extract_install_functions()`, manually sourcing `test_helper.bash` directly in this session's persistent tool shell (outside `bats`) produced an empty `${BASH_SOURCE[0]}` inside the sourced file — causing `FRAMEWORK_ROOT` to resolve to the current scratch directory instead of the framework root, and every downstream path to fail. Isolated with a two-line minimal repro (`source /tmp/probe.sh` echoing its own `BASH_SOURCE[0]`) before concluding it was an environment-specific quirk of this tool's shell, not a bug in `test_helper.bash` itself — confirmed by running the identical code in a fresh `bash -c` subshell, where it resolved correctly, and by the fact that all 38 pre-existing tests already relied on that exact same `FRAMEWORK_ROOT` line and passed when run through `bats` (which has its own controlled `load` mechanism, unaffected by the quirk). The fix was procedural, not code: stop manually sourcing for ad hoc verification, write the real `.bats` file, and run it through `bats` directly — which is what actually matters, since that's how the suite runs in CI and for every future contributor.

### Also Fixed: CODEOWNERS Was Mentioned Exactly Once, in a Source Comment No Setup Team Would Ever Read

Both audit passes converged on the same root limitation (the integrity manifest has no external trust anchor — a PR that edits `gate.sh` *and* regenerates the pin together, outside Claude Code's tool-mediated hooks entirely, still passes CI) and both independently noted the code's own comment already recommends the fix: CODEOWNERS + branch protection. Verified this was never actually surfaced anywhere a human running the setup would see it — grepped the entire repo for "CODEOWNERS" and found exactly one hit, `templates/verify_governance_integrity.sh:23`, a source comment. Added a concrete STEP 3/4 — ACTIVATION section to both implementation packages with the exact `.github/CODEOWNERS` content to create (naming every trust-root path from A6–A10) and the exact GitHub Settings steps to enable branch-protection enforcement of it — plus an explicit note that CODEOWNERS alone is advisory; GitHub only enforces it when a branch protection rule says to. `docs/SECURITY_POSTURE.md`'s threat-model section gained a matching honest-disclosure bullet, in the same voice as the existing `org_policy.json` correction: name the limitation precisely, name the fix, name where the fix lives, don't overclaim it's automatic.

### Verification

Full bats suite: 42/42 passing (16 new tests across two files, zero regressions in the other 26). Every new mechanism was proven against the real artifact it protects, not asserted: the guard tests ran the actual deployed template; the coverage tests ran the actual install.sh functions via robust extraction; the `sed -i.bak` portability claim was verified directly rather than assumed from memory of the classic BSD/GNU trap.

### What Getting Two Differently-Scored Reports Actually Resolved

The two audits disagreed on a number (8.1 vs 8.3) but agreed on substance — both found the same class of remaining gap from different angles (one caught via direct file inspection, one via applying the ownership test one layer further). Given a user who couldn't tell which number was "true," the number was never the useful part of either report — the specific, checkable claims were. Verifying those claims directly (grep for the guard's test coverage, grep for CODEOWNERS mentions) resolved the disagreement without needing to adjudicate whose score was more accurate: both were pointing at real, fixable gaps, and both are now closed.
...
Before evaluating a single new finding, the git history was checked directly: every commit across A6–A11 net-added far more than it removed (1,274 insertions vs. 176 deletions across the fix commits), and every deletion traced to dead code removed *after* extracting shared logic elsewhere — never a weakened check. No regression occurred. The score dropped because this fresh audit examined the *entire* framework — including the original secrets scanner, layer-boundary scanner, and process-ancestry check, none of which the A6–A11 loop had ever touched, because that loop was narrowly re-auditing one specific mechanism (the trust-root deny-list) across six rounds. That mechanism went from wide-open to thoroughly tested; the rest of the framework was never worse than it had always been — it simply wasn't in the spotlight the previous six rounds were pointed at.

### Two Genuinely New, Verified Findings

**1. `_is_claude_agent_process` had a real, previously undocumented bypass.** The existing check only walked the OS process tree looking for a `claude-code` ancestor. An agent that backgrounds a commit (`git commit ... &`) and lets its own tool call return before the commit finishes can sever that live process-tree link — the orphaned child gets reparented (typically to PID 1 or a subreaper) before `gate.sh` ever runs, so the walk finds no agent ancestor at all, silently skipping the AI-only brainstorming and pre-push checkpoints. This is a *different* vector from the one the framework's own curriculum (Module 12.6, written earlier this session) had reasoned about — that module only ruled out renaming a script to fake the ancestor string, never considered detachment.

**Fixed** by checking `$CLAUDECODE` first — an environment variable Claude Code sets in every process it spawns, which is inherited across `setsid`/`nohup`/backgrounding (unlike PPID, which changes on reparenting) — falling back to the process-tree walk only if the env var isn't set. Verified this is a real Claude Code behavior, not an assumption: this very session's own `$CLAUDECODE` was confirmed set to `1` while testing the fix, live.

**Disclosed, not claimed fixed:** an agent that deliberately strips its own environment (`env -u CLAUDECODE` / `env -i`) before running a command defeats this — the same fundamental limit as the Bash-guard hook's lexical scanning (Module A7). The fix closes the realistic accidental-detachment case; it is not a defense against deliberate evasion.

**A genuinely funny verification problem:** testing "a commit with no agent signal at all" from inside an actual Claude Code session run by an actual agent is not really possible — this session's own real process ancestry (the Claude Desktop app's binary path literally contains the string `claude-code`) means the fallback process-walk would correctly detect agent presence *regardless* of what happens to the environment variable. Confirmed this directly by walking the session's own process tree. The corresponding bats test (`tests/gate/agent_detection.bats`) detects this exact condition at runtime and skips gracefully with an explanation, rather than either failing for a confusing reason or asserting something false. It exercises for real in CI or a human-run suite, where no such ancestor exists.

**2. The secrets scanner and layer-boundary scanner had zero adversarial-variant tests — and testing them directly surfaced three additional, real, previously undisclosed gaps in the secrets scanner itself**, not just a testing gap:
- A bare AWS access key (`AKIA...`) or GitHub token (`ghp_...`) with no English keyword ("secret", "password", "token"...) nearby slipped through entirely — the scanner was purely keyword-based.
- A PKCS8-format private key (`-----BEGIN PRIVATE KEY-----`, no algorithm prefix — the modern default format for most tooling, e.g. `openssl genpkey`) also slipped through: the old regex only matched `BEGIN (RSA|EC|OPENSSH|PGP)`, and a bare "PRIVATE KEY" with a *space* (not the underscore/hyphen the `private[_-]?key` keyword pattern expected) didn't match either alternative.

**Fixed** by adding format-based signatures for well-known, high-confidence credential prefixes (AWS, GitHub PAT/fine-grained-PAT, Slack, Stripe) — the same class of signature dedicated tools like gitleaks/git-secrets use — and replacing the algorithm-enumerated PEM pattern with `BEGIN [A-Z ]*PRIVATE KEY`, which covers every PEM variant (RSA/EC/OPENSSH/DSA/ENCRYPTED/PGP/PKCS8) in one pattern instead of enumerating them. Verified all 5 new cases now caught, all 3 original cases still caught (no regression), and the placeholder-exclusion case still passes — before touching the real file.
...
Full bats suite: 54/54 (53 passing, 1 gracefully self-skipping for the reason above), 10 new tests across three files (`agent_detection.bats`, plus additions to `secrets_block.bats` and `layer_boundary_block.bats`), zero regressions in the other 44. Every fix was proven against the real mechanism before being called fixed: the regex improvements were tested against 8 cases (5 new + 3 original) in an isolated scratch repo before ever touching `gate.sh`; the `$CLAUDECODE` behavior was confirmed live in this session's own environment, not assumed from documentation.
