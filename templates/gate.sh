...
# Process-tree lineage check gates enforcement to Claude agent sessions only;
...
# Process-tree lineage check gates enforcement to Claude agent pushes only;
# human pushes bypass this gate entirely to preserve zero-friction velocity.
if [ "$GATE_TRIGGER" = "pre-push" ]; then
    _is_claude_agent_process
    if [ "$IS_AGENT" = "true" ]; then
